...
# def run_cpu_load(core, cpu_load, duration, plot=False, sampling_interval=0.1):
#     if len(cpu_load) > 1 and len(cpu_load) != len(core):
#         raise ValueError("Number of cores and loads does not match.")
#     elif len(cpu_load) == 1:
#         cpu_load = itertools.repeat(cpu_load[0], len(core))

#     core = list(set(core))  # Remove duplicate cores
...
def load_core(target_core, target_load, duration_seconds=-1, plot=False, sampling_interval=0.1):
    if duration_seconds >= 0:
        print(f'Loading core {target_core} ({target_load * 100.0:0>5.2f}%) for {duration_seconds} seconds.')
    else:
        print(f'Loading core {target_core} ({target_load * 100.0:0>5.2f}%) until interrupted.')
    ...
    # polymorphism for plotting
    if plot:
        actuator = PlottingClosedLoopActuator(control, monitor, duration_seconds, target_core)
    else:
        actuator = ClosedLoopActuator(control, monitor, duration_seconds, target_core)
...
def generate_cpu_load(total_load, duration=-1, plot=False, sampling_interval=0.1):
    if plot and duration < 0:
        raise ValueError('Plot option can only be used with a fixed duration.')

    # Get the number of available CPU cores
    available_cores = psutil.cpu_count(logical=False)  # Use physical cores only
    if available_cores is None:
        raise ValueError("Unable to detect the number of available CPU cores.")

    # Divide the total load evenly across all cores
    load_per_core = total_load / available_cores
    ...
    # Spawn one process per core
    with multiprocessing.Pool(len(cores)) as pool:
        pool.starmap(load_core, zip(
            cores,
            itertools.repeat(load_per_core),
            itertools.repeat(duration),
            itertools.repeat(plot),
            itertools.repeat(sampling_interval)
        ))
...
if __name__ == '__main__':
    # This is kept for standalone execution if needed
    generate_cpu_load(total_load=0.2, duration=20, plot=False)

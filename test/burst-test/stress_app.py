...
def cpu_stress(duration, intensity):
    """More reliable CPU stress using multiprocessing."""
...
def stress_core():
    """Simple infinite computation to stress CPU."""
    while True:
        pass

def memory_stress(size):
    """Allocate and hold memory."""
    try:
        return bytearray(size)
    except MemoryError:
        print(f"Memory allocation failed for {size} bytes.")
        return None
...
def run_stress_test(config):
    ...
    for current_time in range(duration):
        ...
        active_bursts = [
            ...
            if current_time >= burst['start'] and current_time < burst['start'] + burst['duration']
        ]
        ...
        # Concurrent CPU and Memory stress
        memory_thread = threading.Thread(target=memory_stress, args=(current_memory,))
        cpu_thread = threading.Thread(target=cpu_stress, args=(1.0, current_cpu))

...
// maxBinaryScanBytes bounds the read-only binary version scans (Node.js release
// URL, Deno/Bun user agents, GraalVM release string).
//
// Sized for the worst known case, Node.js: measured empirically against the
// official node:{18,20,22,24}-slim Docker Hub images, the release-URL string
// sits at 30-45MB into an 91-122MB binary (it scales with binary size, not a
// fixed offset — it is NOT near the head or tail of the file). 64MiB covers all
// four with ~20MB of margin over the largest observed offset. Best-effort: as
// Node's binary keeps growing across releases, this may eventually need to grow
// too — the runtime-metrics-kind CI workflow exercises this against a real
// image on every PR specifically to catch that regression before it reaches
// production. The scan streams in fixed-size chunks (see scanFileSubmatch), so
// this cap bounds I/O, not memory.
const maxBinaryScanBytes = 64 << 20 // 64MiB
...
// resolveNodeVersion best-effort resolves the Node.js version: NODE_VERSION env
// var (official Docker Hub node images), else a streamed scan of the on-disk
// binary for the release-URL string Node embeds for its own process.release
// metadata. Never executes the binary. Returns ("", "") if neither resolves.
...
func scanBinaryVersion(pidDir string, re *regexp.Regexp) (version, source string) {
	...
	if v := scanFileSubmatch(exePath, maxBinaryScanBytes, re); v != "" {

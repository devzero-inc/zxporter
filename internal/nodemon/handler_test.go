...
func (f *fakeQuerier) QueryMetrics(_ context.Context) ([]nodemon.GPUMetric, error) {
	return f.metrics, f.err
}

func sampleMetrics() []nodemon.GPUMetric {
	now := time.Date(2026, 2, 23, 12, 0, 0, 0, time.UTC)
	return []nodemon.GPUMetric{
		{
			NodeName:       "node-1",
			ModelName:      "A100",
			Device:         "nvidia0",
...
func TestContainerMetricsHandler(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := nodemon.NewContainerMetricsHandler(tt.querier, log)
			...
			var result []nodemon.GPUMetricResponse

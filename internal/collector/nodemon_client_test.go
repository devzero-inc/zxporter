...
//nolint:unparam // node is always "node1" in tests, which is intentional for consistency
func makeGPU(
	node, model, device, uuid, pod, container, ns string,
	util, fbUsed, fbFree, power, temp, smClk, memClk float64,
) NodemonMetric {
	return NodemonMetric{
		NodeName:        node,
		ModelName:       model,
		Device:          device,
		DeviceUUID:      uuid,
...
func TestContainerGPUMetricsFromNodemon_Empty(t *testing.T) {
	m := ContainerGPUMetricsFromNodemon(nil, 0, 0)
	if len(m) != 0 {
		t.Errorf("expected empty map for nil input, got %d keys", len(m))
	}

	m = ContainerGPUMetricsFromNodemon([]NodemonMetric{}, 1, 1)
	if len(m) != 0 {
		t.Errorf("expected empty map for empty slice, got %d keys", len(m))
	}
}

func TestContainerGPUMetricsFromNodemon_SingleGPU(t *testing.T) {
	gpus := []NodemonMetric{
...
func TestContainerGPUMetricsFromNodemon_MultiGPU(t *testing.T) {
	gpus := []NodemonMetric{
	...
	m := ContainerGPUMetricsFromNodemon(gpus, 3, 3)
...
func TestContainerGPUMetricsFromNodemon_MixedModels(t *testing.T) {
	gpus := []NodemonMetric{
	...
	m := ContainerGPUMetricsFromNodemon(gpus, 2, 2)
...
func TestContainerGPUMetricsFromNodemon_ZeroUtilization(t *testing.T) {
	gpus := []NodemonMetric{
...
func TestNodeGPUMetricsFromNodemon_Empty(t *testing.T) {
	...
	m = NodeGPUMetricsFromNodemon([]NodemonMetric{})
	if len(m) != 0 {
		t.Errorf("expected empty map for empty slice, got %d keys", len(m))
	}
}

func TestNodeGPUMetricsFromNodemon_SingleGPU(t *testing.T) {
	gpus := []NodemonMetric{
		makeGPU("node1", "T4", "nvidia0", "UUID-1", "pod-a", "vllm", "default",
...
func TestNodeGPUMetricsFromNodemon_MultiGPU_AvgMaxSum(t *testing.T) {
	gpus := []NodemonMetric{
...
func TestNodeGPUMetricsFromNodemon_FourGPUs_MixedModels(t *testing.T) {
	gpus := []NodemonMetric{
		makeGPU("node1", "A100", "nvidia0", "UUID-1", "", "", "",
...
func TestNodeGPUMetricsFromNodemon_MemoryTemperature(t *testing.T) {
	gpus := []NodemonMetric{
		{
...
func TestNodeGPUMetricsFromNodemon_ProfilingMetrics(t *testing.T) {
	gpus := []NodemonMetric{
		{
			NodeName: "node1", ModelName: "A100", Device: "nvidia0",

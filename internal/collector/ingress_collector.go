...
func (c *IngressCollector) Start(ctx context.Context) error {
	...
	c.logger.Info("Starting resources batcher for Ingresses")
	c.batcher.start()
...
func (c *IngressCollector) Stop() error {
	...
	select {
	case <-c.stopCh:
		c.logger.Info("Ingress collector stop channel already closed")
	default:
		close(c.stopCh)
		c.logger.Info("Closed ingress collector stop channel")
	}
	...
	if c.batchChan != nil {
		close(c.batchChan)
		c.batchChan = nil
		c.logger.Info("Closed ingress collector batch input channel")
	}

	// 3. Stop the batcher (waits for completion).
	if c.batcher != nil {
		c.batcher.stop()
		c.logger.Info("Ingress collector batcher stopped")
	}

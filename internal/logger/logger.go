...
func (l *logger) Report(level gen.LogLevel, source string, msg string, err error, fields map[string]string) {
	...
	default:
		...
		l.defaultLogger.Info("Telemetry log queue is full. Dropping log.", "source", source)
	}
}
...
func (l *logger) flush(ctx context.Context, batch []*gen.LogEntry) {
	...
	l.defaultLogger.Info("Flushing telemetry log batch", "batch_size", len(batch))
	...
	if _, err := l.client.SendTelemetryLogs(ctx, req); err != nil {
		l.defaultLogger.Error(err, "Failed to send telemetry logs")
	}
}

// A hashing function for rate limiting
func hash(source, msg, errorString string, fields map[string]string) uint64 {
	h := fnv.New64a()

...
statfunc int get_network_details_from_sock_v4(struct sock *, net_conn_v4_t *,
                                              int);
statfunc struct ipv6_pinfo *inet6_sk_own_impl(struct sock *,
                                              struct inet_sock *);
statfunc int get_network_details_from_sock_v6(struct sock *, net_conn_v6_t *,
                                              int);
statfunc int get_local_sockaddr_in_from_network_details(struct sockaddr_in *,
                                                        net_conn_v4_t *, u16);
statfunc int get_remote_sockaddr_in_from_network_details(struct sockaddr_in *,
                                                         net_conn_v4_t *, u16);
statfunc int get_local_sockaddr_in6_from_network_details(struct sockaddr_in6 *,
                                                         net_conn_v6_t *, u16);
statfunc int get_remote_sockaddr_in6_from_network_details(struct sockaddr_in6 *,
                                                          net_conn_v6_t *, u16);
...
statfunc volatile unsigned char get_sock_state(struct sock *sock) {
  ...
  bpf_core_read((void *)&sk_state_own_impl, sizeof(sk_state_own_impl),
...
statfunc int get_network_details_from_sock_v4(struct sock *sk,
...
statfunc struct ipv6_pinfo *inet6_sk_own_impl(struct sock *__sk,
...
statfunc int get_network_details_from_sock_v6(struct sock *sk,
                                              net_conn_v6_t *net_details,
                                              int peer) {
  // inspired by 'inet6_getname(struct socket *sock, struct sockaddr *uaddr, int
  // peer)' reference:
  // https://elixir.bootlin.com/linux/latest/source/net/ipv6/af_inet6.c#L509
  ...
  // the flowinfo field can be specified by the user to indicate a network flow.
  // how it is used by the kernel, or whether it is enforced to be unique is not
  // so obvious.  getting this value is only supported by the kernel for
  // outgoing packets using the 'struct ipv6_pinfo'.  in any case, leaving it
  // with value of 0 won't affect our representation of network flows.
  ...
  // iface)' function.  in any case, leaving it with value of 0 won't affect our
...
statfunc int get_local_sockaddr_in_from_network_details(
...
statfunc int get_remote_sockaddr_in_from_network_details(
    struct sockaddr_in *addr, net_conn_v4_t *net_details, u16 family) {
  addr->sin_family = family;
...
statfunc int get_local_sockaddr_in6_from_network_details(
...
statfunc int get_remote_sockaddr_in6_from_network_details(
    struct sockaddr_in6 *addr, net_conn_v6_t *net_details, u16 family) {
  addr->sin6_family = family;
...
statfunc bool fill_tuple(struct sock *sk, tuple_t *tuple) {
  ...
  case AF_INET:
    ...
    break;
  case AF_INET6:
    BPF_CORE_READ_INTO(&tuple->saddr.u6_addr32, sk,
                       __sk_common.skc_v6_rcv_saddr.in6_u.u6_addr32);
    BPF_CORE_READ_INTO(&tuple->daddr.u6_addr32, sk,
...
statfunc bool fill_tuple_from_bpf_sock(struct bpf_sock *sk, tuple_t *tuple) {
  ...
  case AF_INET:
    ...
    break;
  case AF_INET6:
    // __builtin_memcpy is not compatible with bpf_sock so we have to copy the
    // fields
  ...
  tuple->dport =
      bpf_ntohs(sk->dst_port); // Convert to host byte order (little endian).
...
statfunc bool is_addr_public(const addr_t *addr, __u16 family) {
  ...
  case AF_INET6: {
    ...
    // Site-local addresses fec0::/10 (deprecated, but still checked for
    // coverage)

...
| `dcgmExporter.runtimeClassName` | RuntimeClass for the GPU DaemonSet only. `auto` uses the `nvidia` RuntimeClass if the cluster has one. `""` renders the original single DaemonSet without a RuntimeClass. | `auto` |
| `dcgmExporter.gpuNodeSelectorTerms` | Node selector terms used to scope the RuntimeClass DaemonSet to GPU nodes when split mode is active. Defaults cover NVIDIA GPU Operator, GKE, and AKS GPU labels. | see `values.yaml` |
...
`dcgmExporter.runtimeClassName: auto` uses Helm `lookup`, so it only detects
RuntimeClasses during a live Helm install or upgrade. Static YAML generated by
`helm template` renders the original single DaemonSet unless a RuntimeClass is
explicitly set at render time.
